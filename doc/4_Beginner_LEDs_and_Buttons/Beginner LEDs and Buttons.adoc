...
TODO: add a note to create a new branch for this lesson and push your changes to it when you are done

TODO: no need to fetch upstream?
=== Setup
...
2. Create a new branch for lesson 6 using `git checkout -b lesson-3`
...
TODO: make this screenshot and example not use buttons
...
TODO: fix paragraph whitespace syntax below
TODO: don't mention anything about device tree in this lesson

TODO: don't mention interrupts in this lesson, just polling
...
2. Interrupts:
   The robust solution. This involves writing a special function, and then putting that function into the post-processing slot of a hardware event trigger. This "interrupt function" is called
   an *Interrupt Service Routine* (ISR). 
   * different hardwares have different available events that can trigger an interrupt, but for GPIO inputs (like buttons) the most common is a high/low edge trigger. This means that when the button is pressed or released, it will trigger an interrupt.
   * When the event you specified occurs, the processor will
   ...
   always being active and reading an input. 
   * With an interrupt-based program, you can put the
...
the interrupt, but sometimes they don't, and that's why it only prints multiple times sometimes.
